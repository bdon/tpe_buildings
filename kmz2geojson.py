...
def makeShape(l,height,lon,lat):
  ...
  all_feats.append({'type':'Feature','properties':{'height':height},'geometry':{'coordinates':[geog],'type':'Polygon'}})
  ...
  with ZipFile(region,'r') as myzip:
    ...
    for placemark in xpath(root,"n:Document/n:Placemark",nskml):
      ...
      for inst_geom in model.findall('.//{http://www.collada.org/2005/11/COLLADASchema}instance_geometry'):
        ...
        points = grouper(3,vals)

        prev_height = None
        poly = []
        for point in points:
          if point[2] != prev_height and prev_height != None:
            makeShape(poly,prev_height,lon,lat)
            poly = []
          prev_height = point[2]
